...
# Version 3 of pong. In this version a ball spawns in a random location in the window moving in a random direction. Two paddles also spawn , the ball bounces off of the front of the paddles and the walls. The paddles can be moved up and down and are bounded by the window. Paddle1 moves up and down by pressing 'w' or 's' and paddle2 moves up and down by pressing the up arrow key and down arrow key. The game terminates upon closing the window or upon either player reaching a score of 11 otherwise it will continue indefinitely.
...
class Game():
    ...
    def draw_score(self):
        ...
        text1_img   = font.render(str(self.score1),True, font_color, font_bg)     
        text2_img = font.render (str(self.score2),True,font_color,font_bg)
        text1_pos   = (0,0)
        text2_pos = (470,0)
        self.surface.blit(text1_img, text1_pos)
        self.surface.blit(text2_img,text2_pos)
    ...
    def handle_events(self):
        ...
        # move up bounded by top (paddle1)
        if self.Paddle1.rect[1]>=0: 
            if pressed[pygame.K_w]: 
                self.Paddle1.move_ip(0,-2)
        # move down bounded by bottom (paddle1)    
        if self.Paddle1.rect[1]+self.Paddle1.rect[3]<=400:    
            if pressed[pygame.K_s]: 
                self.Paddle1.move_ip(0,  2)
        # move up bounded by top paddle 2        
        if self.Paddle2.rect[1]>=0:    
            if pressed[pygame.K_UP]: 
                self.Paddle2.move_ip(0, -2)
        # move down bounded by bottom paddle2        
        if self.Paddle2.rect[1]+self.Paddle2.rect[3]<=400: 
...
class Rect:
    ...
    def draw(self):
        pygame.draw.rect(self.surface,pygame.Color('white'),self.rect)
